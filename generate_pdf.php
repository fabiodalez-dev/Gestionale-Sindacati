...
<div class="container">
  ...
  <table class="zebra-table">
    <tr>
      ...
      <td>' . htmlspecialchars($lavoratore['nome_azienda'] ?? '') . '</td>
    </tr>
    <tr>
  ...
  <div class="comunicazione">
    <p>
      Con la presente comunica che ha ceduto all\'' . htmlspecialchars($settings['nome_completo'] ?? '') . ', parte del suo credito retributivo vantato nei confronti della Vostra Azienda, pari allo 0,80% della retribuzione base mensile (paga base + contingenza, scatti di anzianità, EDR, con riproporzionamento nei part-time) per 13 mensilità di retribuzione annue.
    </p>
    <p>
      Resta quindi convenuto che viene a cessare qualsiasi titolo del/la sottoscritto/a della presente in relazione ai suddetti crediti come innanzi individuati e specificati. Ogni qualsiasi pagamento, in relazione e limitatamente ai sopraccitati crediti, per essere per voi liberatorio (art. 1264 C.C.) dovrà essere esclusivamente effettuato a favore della:
    </p>
    <p class="center bold">
      ' . htmlspecialchars($settings['nome_completo'] ?? '') . '<br>
      ' . htmlspecialchars($settings['denominazione_e_iban'] ?? '') . '
    </p>
    <p>
      quale unico acquirente e titolare dei crediti medesimi. 
      La presente notificazione viene effettuata dallo/a scrivente tanto ai sensi degli articoli 1260 e SS.CC., quanto al vigente Contratto Collettivo Nazionale del Lavoro.
    </p> 
    <p class="bold">
      Inoltre, con la firma della presente il/la sottoscritto/a revoca ogni adesione di cessione rilasciata ad Organizzazioni Sindacali diverse da ADL e, pertanto, dispone affinché la società in indirizzo cessi, a partire dalla data della sottoscrizione, di operare ogni trattenuta di quota sindacale o cessione a favore di altra organizzazione sindacale diversa da ADL.
    </p>
    <p>
      Ricevuta la informativa sulla utilizzazione dei miei dati personali ai sensi della art.10 L.875/96, acconsente al trattamento necessario agli scopi statutari.
    </p>
  </div>
</div>
</body>
...
<table width="100%" style="border-top:1px solid #000;font-size:10pt;margin-top:20px;">
  ...
  <tr>
    <td colspan="2" style="text-align:center;padding-top:5px;font-weight:bold;">DA CONSEGNARE ALLA SOCIETÀ</td>
  </tr>
</table>';
...
$nome_file = preg_replace('/[^a-zA-Z0-9_-]/', '', $lavoratore['cognome']) . '_' . preg_replace('/[^a-zA-Z0-9_-]/', '', $lavoratore['nome']) . '.pdf';
