            ...
            if ($handle !== false) {
                ...
                if ($header === false) {
                    ...
                    while (($data = fgetcsv($handle, 1000, ',', '"')) !== false) {
                        ...
                        $azienda_nome = isset($data[0]) ? trim($data[0]) : '';
                        $cognome_e_nome = isset($data[1]) ? trim($data[1]) : '';
                        $codice_fiscale = isset($data[2]) ? trim($data[2]) : '';
                        $telefono = isset($data[3]) ? trim($data[3]) : '';
                        $sesso = isset($data[4]) ? trim($data[4]) : '';
                        $comune_residenza = isset($data[5]) ? trim($data[5]) : '';
                        $provincia = isset($data[6]) ? trim($data[6]) : '';
                        $paese_nascita = isset($data[7]) ? trim($data[7]) : '';
                        $data_iscrizione = isset($data[8]) ? trim($data[8]) : '';
                        $settore = isset($data[9]) ? trim($data[9]) : '';

                        // Controlla che 'cognome_e_nome' non sia vuoto
                        ...
                        $settore = 'Privato'; // Imposta il settore come 'Privato' per tutti
                        ...
                        // Correggi 'PERU'' in 'Perù' nel 'paese_nascita'
                        if (strtoupper($paese_nascita) === "PERU'") {
                            $paese_nascita = "Perù";
                        }

                        // Formatta la data 'DATA ISCRIZIONE'
                        if (!empty($data_iscrizione)) {
                            $date_parts = explode('/', $data_iscrizione);
                            if (count($date_parts) === 3) {
                                $month = str_pad($date_parts[0], 2, '0', STR_PAD_LEFT);
                                $day = str_pad($date_parts[1], 2, '0', STR_PAD_LEFT);
                                $year = str_pad($date_parts[2], 2, '0', STR_PAD_LEFT);
                                // Aggiusta l'anno se è a due cifre
                                if (strlen($year) === 2) {
                                    $year = '20' . $year;
                                }
                                $data_iscrizione_formattata = "$year-$month-$day";
                            } else {
                        ...
                        // Controllo duplicati
                        // Definisci una chiave univoca per il lavoratore, ad esempio combinazione di nome, cognome, azienda, data di iscrizione
                        ...
                        // Prepara la query di inserimento, gestendo il caso in cui 'codice_fiscale' sia vuoto
                        if (!empty($codice_fiscale)) {
                            $stmt = $mysqli->prepare("INSERT INTO lavoratori (nome, cognome, codice_fiscale, indirizzo_citta, indirizzo_provincia, telefono, paese_nascita, data_iscrizione, settore, genere, azienda_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                            $stmt->bind_param(
                                'ssssssssssi',
                                $nome,
                                $cognome,
                                ...
                                $telefono,
                                $paese_nascita,
                                $data_iscrizione_formattata,
                                $settore,
                                $genere,
                                $azienda_id
                            );
                        } else {
                            $stmt = $mysqli->prepare("INSERT INTO lavoratori (nome, cognome, indirizzo_citta, indirizzo_provincia, telefono, paese_nascita, data_iscrizione, settore, genere, azienda_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                            $stmt->bind_param(
                                'sssssssssi',
                                $nome,
                                $cognome,
...
<head>
    <meta charset="UTF-8">
    <title>Importa Lavoratori</title>
    <!-- Font Awesome -->
...
<body>
    <div class="container mt-5">
        <h1>Importa Lavoratori da CSV</h1>
        <?php if (isset($error)): ?>
        ...
        <?php endif; ?>
        <form action="import_lavoratori.php" method="post" enctype="multipart/form-data">
